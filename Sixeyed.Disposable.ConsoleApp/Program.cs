...
namespace Sixeyed.Disposable.ConsoleApp
{
    class Program
    {
        ...
        {
            const string bookpath = @"C:\Users\Sean\Source\Repos\PluralSight_IDisposableBestPractices\Sixeyed.Disposable\books";
            var archiver = new FolderWatcher();
            archiver.Start( bookpath, "*.txt", ProcessFile);
            Console.WriteLine($"Listening on {bookpath}");
        }
        ...
        private static void ProcessFile(string path)
        {
            ...
            var archivePath = string.Format(@"{1}\archive\{0}", Guid.NewGuid(), bookpath);
           // Console.ReadLine();
